...
extend(MBlockly.Control, {
    ...
    // 主板类型为：mcore,orion,auriga,megaPi,aircar,airdrone,aircustom
    deviceInfo : {
        type: "auriga",
...
extend(MBlockly.Control, {
    ...
    setMbotLed: function(r, g, b, position) {
        ...
        // if (position == 1) {
        //     this.setLedByPosition(0, 0, 0, 2, port);
        // } else if (position == 2) {
        //     this.setLedByPosition(0, 0, 0, 1, port);
        // }
    },
...
extend(MBlockly.Control, {
     ...
     *     6: long(4 byte)
     */
    sensor_callback: function(type) {
        console.log(type + ' callback: ' + this.buffer.join(' '));
        // 获取返回的数据类型
        var dataType = this.buffer[3];
        if (this.buffer[0] == 0xff && this.buffer[1] == 0x55) {
            var result = null;
            switch(dataType) {
                case "1":
                    // 1byte
                    result = this.buffer[4];
                    break;
                case "3":
                    // 2byte
                    result = this.calculateResponseValue([parseInt(this.buffer[5]), parseInt(this.buffer[4])]);
                    break;
                case "4":
                    // 字符串
                    result = this.bytesToString(this.buffer);
                    break;
                case "2":
                case "5":
                case "6":
                    // long型或者float型的4byte处理
                    result = this.calculateResponseValue([parseInt(this.buffer[7]), parseInt(this.buffer[6]), parseInt(this.buffer[5]), parseInt(this.buffer[4])]);
                    break;
                default:
                    break;
            }
            ...
            MBlockly.Control.PromiseList.receiveValue(this.buffer[this.SETTING.READ_BYTES_INDEX], result);
        }
    },
